...
uint32_t __stdcall stub_AvSetDisplayMode(void* RegisterBase, uint32_t Step, uint32_t Mode, uint32_t Format, uint32_t Pitch, void* FrameBuffer) { return 0; }
...
uint32_t __stdcall stub_ExQueryNonVolatileSetting(uint32_t ValueIndex, uint32_t* Type, void* Value, uint32_t ValueLength, void* ResultLength) { return 0; }

// === Hal ===

// 0x002C - #44 HalGetInterruptVector
uint32_t __stdcall stub_HalGetInterruptVector(uint32_t BusInterruptLevel, void* Irql) { return 0; }
...
uint32_t __fastcall stub_InterlockedExchange(void* Destination, uint32_t Value) { return 0; }

// === Io ===

// 0x0043 - #67 IoCreateSymbolicLink
uint32_t __stdcall stub_IoCreateSymbolicLink(void* SymbolicLinkName, void* DeviceName) { return 0; }

// 0x0045 - #69 IoDeleteSymbolicLink
uint32_t __stdcall stub_IoDeleteSymbolicLink(void* SymbolicLinkName) { return 0; }

// === Ke ===

// 0x0062 - #98 KeConnectInterrupt
uint32_t __stdcall stub_KeConnectInterrupt(void* InterruptObject) { return 0; }

// 0x0063 - #99 KeDelayExecutionThread
uint32_t __stdcall stub_KeDelayExecutionThread(uint32_t WaitMode, uint32_t Alertable, void* Interval) { return 0; }

// 0x0067 - #103 KeGetCurrentIrql
uint32_t __stdcall stub_KeGetCurrentIrql(void) { return 0; }

// 0x0068 - #104 KeGetCurrentThread
void* __stdcall stub_KeGetCurrentThread(void) { return 0; }
...
uint32_t __stdcall stub_KeSetBasePriorityThread(void* Thread, uint32_t Priority) { return 0; }
...
void* __stdcall stub_MmAllocateContiguousMemory(uint32_t NumberOfBytes) { return 0; }

// 0x00A6 - #166 MmAllocateContiguousMemoryEx
void* __stdcall stub_MmAllocateContiguousMemoryEx(uint32_t NumberOfBytes, uint32_t LowestAddr, uint32_t HighestAddr, uint32_t Alignment, uint32_t ProtectionType) { return 0; }
...
uint32_t __stdcall stub_ObReferenceObjectByHandle(uint32_t Handle, void* ObjectType, void** ReturnedObject) { return 0; }
...
uint32_t __stdcall stub_PsCreateSystemThreadEx(void* ThreadHandle, uint32_t ThreadExtensionSize, uint32_t KernelStackSize, uint32_t TlsDataSize, void* ThreadId, void* StartRoutine, void* StartContext, uint32_t CreateSuspended, uint32_t DebugStack, void* SystemRoutine) { return 0; }
